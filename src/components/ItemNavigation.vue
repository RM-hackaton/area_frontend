<script setup>
import {defineProps, ref} from 'vue'
import {useRouter} from 'vue-router'

const router = useRouter()

const navBlocks = ref([])
const title = ref(null)

const props = defineProps({
	title: String,
	white: {
		type: Boolean,
		default: false
	}
})

</script>
<template>
<div :class="{'text-white': props.white}" @click="router.back()" class="pb-10 text-lg back flex w-full justify-center items-center">
	<svg class="mr-4" width="10" height="15" viewBox="0 0 6 10" fill="none" xmlns="http://www.w3.org/2000/svg">
	<path d="M1.14821 5L5.39377 0.636508C5.5382 0.48807 5.53495 0.250655 5.38651 0.106228C5.23807 -0.038199 5.00065 -0.0349467 4.85623 0.113492L0.356228 4.73849C0.214591 4.88406 0.214591 5.11594 0.356228 5.26151L4.85623 9.88651C5.00065 10.0349 5.23807 10.0382 5.38651 9.89377C5.53495 9.74935 5.5382 9.51193 5.39377 9.36349L1.14821 5Z" :fill="props.white ? '#ffffff' : '#222222'"/>
	</svg>
	<span>вернуться назад</span>
</div>
<h1 class="text-3xl font-extrabold" :class="{'text-white': props.white}">{{ props.title }}</h1>

</template>
<style>

</style>
